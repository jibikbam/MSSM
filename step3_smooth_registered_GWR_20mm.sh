...
smooth(){
    ...
    for file in `ls $SUBJECTS_DIR/$subj/mri/GWC/reg_surf2fsaverage/*$1* | xargs -n 1 basename`; do
        mri_surf2surf   --s fsaverage \
                        ...
                        --trgsurfval $SUBJECTS_DIR/$subj/mri/GWC/reg_surf2fsaverage_smoothed20mm/$file \
                        --fwhm-trg 20 \
                        --hemi "$1" &
    done
}
...
if [[ $subj == $SUBSTRING ]]; then # optional
    ...
    all_files_exist=1
    while read file; do
        if [ ! -e $SUBJECTS_DIR/$subj/mri/GWC/reg_surf2fsaverage_smoothed20mm/$file ]; then
    ...
    if [ $all_files_exist == 1 ]; then
        ...
        mkdir -p $SUBJECTS_DIR/$subj/mri/GWC/reg_surf2fsaverage_smoothed20mm
        smooth "lh"
        cnt=$((cnt+14))
    fi
